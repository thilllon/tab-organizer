...
| **▸ Gather all windows**      | One keyboard shortcut pulls every other window's tabs into the current one. Tabs are moved, never reopened, so pinned tabs, groups, history and typed text stay as they were.                                                                                                                                                         |
...
2. **Keyboard shortcuts** — _Save the current window as a session_ and _Open the Sessions dashboard_ (plus _Move the tabs of all other windows into this window_) are registered as Chrome commands with no preset keys; bind them at `chrome://extensions/shortcuts` (there is a button for it in Options).
