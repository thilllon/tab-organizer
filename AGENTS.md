...
Gathering every window into the focused one (`assemble-tabs` command, no menu item or UI):

```
Keyboard shortcut the user bound at chrome://extensions/shortcuts
...
-> per source window, planWindowMoves() (pure) then, each step appending (index: -1):
   pinned tabs: chrome.tabs.move(ids) then chrome.tabs.update(id, { pinned: true }) in order
   ungrouped runs: chrome.tabs.move(ids); groups: chrome.tabGroups.move(groupId)
   (a failing window is recorded and skipped; the rest continue; emptied windows close themselves)
-> restoreTargetView(): re-activate the recorded active tab, re-collapse groups that arrived collapsed
...
| Listener                                        | Behaviour                                                                                                                                                                                                                                                                                                                                                                                                   |
| ----------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `runtime.onInstalled`                           | `clearBadge()`; `contextMenus.removeAll()` then create the 3 menu items plus a separator (idempotent); on `details.reason === 'update'` runs `sessionRepo.migrateAll()` first (errors caught locally); then `sessionRepo.reconcile()`; then `ensureHistoryAlarm()` (Chrome drops every alarm on update/reload).                                                                                             |
