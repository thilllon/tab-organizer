import type { DuplicateTabHandling, GroupFrom, GroupingMode, SortSettings } from '@/types'
...
function sortByTitleOrUrl(
  ...
  const titleComparator = (a: chrome.tabs.Tab, b: chrome.tabs.Tab): number => {
    ...
    if (groupSuspendedTabs) {
      if (isSuspended(a) && !isSuspended(b)) {
        return -1
      }
      if (!isSuspended(a) && isSuspended(b)) {
        return 1
...
function sortByCustom(
  ...
  if (groupFrom === 'leftToRight') {
    ...
    while (left !== right) {
      ...
      updateTabGroupMap(tabGroupMap, tabs[left], 'custom', groupSuspendedTabs, groupingMode)
      left += 1
    }
  } else {
    while (left !== right) {
      ...
      updateTabGroupMap(tabGroupMap, tabs[right], 'custom', groupSuspendedTabs, groupingMode)
    }
    if (groupSuspendedTabs) {
  ...
  if (groupSuspendedTabs) {
    ...
    if (groupFrom === 'leftToRight') {
      while (left !== right) {
        updateTabGroupMap(tabGroupMap, tabs[left], 'custom', false, groupingMode)
        left += 1
      }
    } else {
      while (left !== right) {
        right -= 1
        updateTabGroupMap(tabGroupMap, tabs[right], 'custom', false, groupingMode)
      }
    }
...
function extractGroupingKey(hostname: string, mode: GroupingMode): string {
  if (mode === 'subdomain') {
    return hostname
  }
